...
var trauma_power : int = 2.0 # Trauma exponent. Increase for more extreme shaking


func _ready() -> void:
	#? Randomize the game seed
	randomize()
	GameManager.on_ball_collide.connect(on_ball_collide)
...
func add_trauma(amount : float) -> void:
	...
	trauma = clamp(amount, 0.0, 1.0)  # 直接设置 trauma 为传入值，范围限制在 [0, 1]
